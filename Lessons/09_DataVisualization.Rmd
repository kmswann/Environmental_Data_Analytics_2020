...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  #LTY IS PATTERN OF LINE
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + #HORIZONTAL LINES ON TOP IN THE CODE FOLLOWED BY POINTS BECAUSE GGPLOT WILL PROJECT THE POINTS AFTER THE LINES - YOU WANT THE POINTS ON TOP VISUALLY.
  #ALPHA TRANSPARENCY 
  ...
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold") + #NOTE THAT THE TEXT IS FORMATED BY X AND Y AND ARE JUST LABELS PLACED ONTOP OF THE FIGURE 
  #HJUST = 1 MEANS...??? XXhorizontal adjustment of 1
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + #SHOWS THE X VALUES AS DATES GIVEN HERE 
  #%b = three leter month %y = 2 digit year
  ylab(expression("O"[3]* " AQI Value")) + #"O"[3]* creates a subscripted 3 next to O 
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = "tp",micro,"g/l", y = "tn", micro, "g/l", color = depth, shape = lakename)) +
...
Nutrientplot <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = , y = expression(paste("Concentration (" ,mu,"g/L)"))), color = nutrient) +
  geom_boxplot() +
