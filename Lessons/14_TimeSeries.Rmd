...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], frequency = 365) #8th column of dataset (discharge_mean)
```
...
#Not a strong seasonal trend; 365 day repeat, 35 years, each year is exactly the same, if it was a strong trend, the repeat would look less noisy and the scale would be much larger, how often do trends repeat? Not actually seasonal...just about cycles...


# We can extract the components and turn them into data frames
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3]) #all rows, columns 1:3 
...
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 1) +
...
Monotonic example: climate change
Step trend example: policy change
...
```{r}
EnoDischarge.early.subsample <- sample_n(EnoDischarge.early, 5000) #5000 random samples
...
#P is sig, not normal distribution - no ttest allowed

var.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean)
#p-value sig, totally unequal variance between the samples - no t-test allowed

wilcox.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean)
#non-parametric: two are significantly different, p-value sig. 
...
> Means and medians are sign different as shown by wilcox test (wILCOXON TEST, w = 1.17*10^8, P<0.0001). 
Show boxplot, violin plot, density plot of data, but will be hard to graph together bc you separated them... 
Note that time is not an explanatory variable here. Instead it's just comparing two periods. 
...
> aggregation: month to year

> subsampling: year to month, etc

> interpolation: filling in data gaps 
...
Temporal autocorrelation: samples next to each other in time will autocorrelate - today's weather impacts tomorrow's weather, etc. 
...
#mean better than median for discharge bc it pulls outlyer data into it. 
#months don't have even # of days, so you have to be careful if you aggregate instead of subsample...
  
EnoDischarge.late.monthly$Date <- as.Date(paste(EnoDischarge.late.monthly$Year, 
                                                EnoDischarge.late.monthly$Month, 
                                                1, sep="-"), 
                                          format = "%Y-%m-%d")
#You have to format the data as a date, so you do it with this hacky code with a "1" and a 1 in order to set the 1st day of the month for each month... in hw... 
...
> Discharge is not following a seasonal monotonic trend from 1985-2019 (. 
