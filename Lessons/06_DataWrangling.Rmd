...
#where depth is equal to zero within the dataset - in matrix notation you need the , at the end to signify matrix notation (to pull row data for all columns)

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) # same function as above but easier to think through
...
#filter is horizontal, #select is vertical; to exclude lakename, write as -lakename
#??????????????????????????????????????????????????????????????????????rearranges columns? 
```
### Mutate
...
#the %% means apply to all of the above... 
