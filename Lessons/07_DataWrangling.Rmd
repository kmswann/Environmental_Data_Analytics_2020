...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #droplevels removes 0s from dataset
summary(NTL.nutrient.data.PeterPaul$lakename)
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #
  mutate(month = month(sampledate)) %>% # month()will find month in the date column
  select(lakeid:daynum, month, sampledate:comments) %>% # 
  drop_na(depth) #drops na values from a given column.
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.subset, "nutrient", "concentration", tn_ug:po4) #can flip a column into a row!!!
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #then removes NA's !is.na()
...
NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration) #opposite of gather, separates out columns into rows
...
* `inner_join`: return rows in x where there are matching values in y, and all columns in x and y (mutating join). [X on left, y on right?]
...
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset) #looks for columns in common then joins
...
#TIME CONSUMING bc you have to open each dataset. 
```
...
Litter <- LitterFiles %>% #note that the pipe here allows you to use read.csv to read all the csvs.
  ldply(read.csv) #creates litter dataframe for everything. 
#leftjoin vs fulljoin is important
```
...
#look at the dimensions, which is the longest? Use the left join, with the longer dimensions being the 'left' listed column....ie litter gets listed first, then trap... 

dim(Litter)
dim(Trap)
dim(LitterTrap)
#19 + 20 columns, minus the 3 columns that we joined by c("site id"...)
