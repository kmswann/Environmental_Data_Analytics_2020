...
vector1[3] # puts the third value out from the list

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #byrow flips the distribution so that the numbers go up by row instead of column 
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) # names the rows and columns 

matrix1[4, ] # gives the row
matrix1[ , 3] # gives the column
matrix1[c(12, 14)] #gives objects 12 and 14
matrix1[c(12:14)] #gives objects 12-14
matrix1[2:4, 1:3] #gives the rows and columns, but renames the row/column titles associated... 

cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # rnames is row names cnames is column names when you call on them with dimnames
matrix4

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1 # semicolon allows you to keep going without two lines; note that the list is really a collection of very different data types. 
list1[[2]] #second part of the 'list'

# Data Frames ----
d <- c(1, 2, 3, 4) # numeric
e <- c("red", "white", "red", NA) # character or string
f <- c(TRUE, TRUE, TRUE, FALSE) # logial
dataframe1 <- data.frame(d,e,f) #dataframe has to have same # of rows
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #opens the dataframe 
...
> Answer: It allows you to pick the data by name.
...
```{r}
highf <- c(62,45,50,50,40,40,45,52,54,58,56,53,49,47,48)
lowf <- c(31,29,43,24,23,23,26,30,42,41,37,36,31,31,30)
highc <- c(17,7,10,10,4,4,7,11,12,14,14,12,9,8,9)
lowc <- c(-1,-1,6,-4,-5,-5,-3,-1,5,5,3,2,-1,-1,-1)
```

Combine your four vectors into a data frame and add informative column names.

```{r}
dframe <- data.frame(highf,lowf,highc,lowc)
names(dframe) <-c("highf","lowf", "highc","lowc"); View(dframe)
```
