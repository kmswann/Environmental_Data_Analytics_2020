...
```{r}
#what the hell are random effects??
TempTest.mixed <- lme(data = NTL.summertemp, #lme is linear mixed effects
                     temperature_C ~ year4, #temp by year (main effect)
                     random = ~1|lakename) #random variable is 1 grouped by lakename the 1 is just a substitute for what would otherwise be called out as 'time'. 
summary(TempTest.mixed)
rsquared(TempTest.mixed)

# Results: random effect will hopefully make your r2  better in your model... 
#Marginal r2 = fixed effect only, conditional r2 = random effect included

#random effects have to be categorical 

# Compare the random effects model with the fixed effects model
TempTest.fixed <- gls(data = NTL.summertemp, #use gls for different output 
                      temperature_C ~ year4)
summary(TempTest.fixed)

#run anova on the rem vs fem 
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  geom_point() +
  scale_color_viridis_d() +
  geom_abline(intercept = -97.72, slope = 0.06) + #from mixed effect output
  # make it look better
  labs(x = "", y = expression("Temperature " ( degree~C)), color = "") +
  theme(legend.spacing.x = unit(0, "cm")) #makes labels closer together.
print(NTL.tempmodel)

#shows fixed effects best fit line vs the radom effect best fit line
NTL.tempmodel2 <-
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C)) +
  geom_point(aes(color = lakename)) +
  geom_smooth(method = "lm", se = FALSE)+ #fixed effects model
  scale_color_viridis_d() +
  geom_abline(intercept = -97.72, slope = 0.06) + #from mixed effect output
  # make it look better
  labs(x = "", y = expression("Temperature " ( degree~C)), color = "") +
  theme(legend.spacing.x = unit(0, "cm")) #makes labels closer together.
print(NTL.tempmodel2)
```

Question: How would you interpret the collective results of your mixed effects model in the context of the study question?

> ANSWER: For each year, there is a 0.06 degree change in temp, over time this is substantial! Note that the slope of 0.06 is significant in the model...
