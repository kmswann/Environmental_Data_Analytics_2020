...
dev.off()
# way to check to make sure pdf is saving to outputs folder. 
# 
```
...
[Go to website, enter in your projection (or search for it), then copy the 'description' section...more useful to grab EPSG code if you can find it.]
...
my_tabular_data_raw <- read.csv(here("data", "tabular_data", "conterm_basinid.txt")) #can also try read.table
...
head(basins_nf_seplains_raw) #prints first several lines
...
#note that latitude and long has been replaced with geometry...
...
NC <- southeast_state_bounds %>%
  select(southeast_state_bounds$NAME == North Carolina)%>%
st_drop_geometry(NC)
  select(southeast_state_bounds_geometry)
# check
# add your code here
