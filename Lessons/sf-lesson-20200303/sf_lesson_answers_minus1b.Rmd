...
# add your code here
regional_basins_nf_seplains <- basins_nf_seplains %>%
  st_overlaps(nc_state_bounds_geom)%>%
  st_transform(4326)

# look at result
# add your code here
head(regional_basins_nf_seplains)
...
# add your code here
regional_gages <- gages %>%
  st_overlaps(regional_basins_nf_seplains)%>%
  st_transform(4326)
...
# plot figure 1b
# add your code here
pdf(here("outputs", "1b"), width = 11, height = 8.5)
ggmap(nc_basemap)+
  geom_sf(data = nc_state_bounds_geom_wsg84, fill = NA, lwd = 1, inherit.aes = FALSE)+
  geom_sf(data = regional_basins_nf_seplains, alpha = 0.25)+
  geom_sf(data = regional_gages, lwd=1)+
  theme_bw()
dev.off()
```
