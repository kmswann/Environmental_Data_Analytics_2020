...
PMplot <- 
  ...
  geom_point()
print(PMplot) #shape = as.factor(year) makes each year a different shape - has to have as.factor because shape applies to categorical data -converting date to category, and color = site.name will make a different color per site.name 
...
print(PMplot.faceted) #facets makes different graph for each site, you can change the scale on each facet's axes  - see facet_wrap help
...
#or ggplot(subset(EPAair, Site.Name %in% c("Clemmons Middle","Leggett","Bryson City"))...

#Consider for MP low/high TP/other criteria by ecoregion, EPA region, nationally...
...
# Adjust axes to exclude unrealistic outliers 
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm #loess smoothing function with CI of the model; phosphorous limited system, with higher line than the expected abline... 
  geom_abline(aes(slope = 16, intercept = 0)) #slope of 16 is the redfield ratio N:P ratio 
print(NvsP)
...
DON'T USE BOX PLOTS UNLESS IT'S COUNT DATA!!!
...
print(Nutrientplot4) #dots scatter horizontally when they're around the same spot. Could be good for MP too.
