...
Cannot run this code because the path is set to a specific machine and user
...
```{r, message = FALSE} 
# if you get an error message when you knit, look for this
# Load package
library(tidyverse)
```
...
# Delete everything up to the "./" then hit tab, and it'll theoretically give you a list of all the file options...but it doesn't here. Changed the knit director to project directory but still didn't 9work. 
# two .. would go outside the folder (up the file directory)
# one . goes inside the folder (down the file directory)
# ~ will take you to your c drive
#././ goes in two folders

# What happens if we don't assign a name to our imported dataset?
#read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv")

#removed the second dot and ran it; it prints out the data, you have to actually name it to call it for manipulation ; put a # infront of the code so it's not printed in the knit pdf. 

# Another option is to choose with your browser
# read.csv(file.choose()) 
#not reproducible because it doesn't track what file you chose. don't use it. 
...
> ANSWER: File found in metadata folder. Tells you who made the csv, when the data was pulled, where from, what each column means, contact info, etc. Note the naming conventions and file formats. 
...
#it's a dataframe, which means it's got a mix of data, it's 2D (not an array) 
colnames(USGS.flow.data)
# lists out the column names but you can change the function so that it renames everything (you use the assign function and apply it to the colnames fucntion)
# Rename columns
...
# add [] with #s for # of given column following ".data)" to rename specific columns. 
# only go 80 characters in on the line before hitting enter to make things easy to read. 

str(USGS.flow.data)
#structure gives basic descriptions of each column, with a lot of NAs for blank cells.BUT some columns are left blank when there are factors (characters) (A, P, and blank). 

dim(USGS.flow.data)
length(USGS.flow.data)
#length gives # columns; width is not a thing; nrow is # rows. 

head(USGS.flow.data)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
#to see how old data compares to new data (not pretty tho)

USGS.flow.data[30000:30005, c(3, 8, 14)]
#subsect the data

class(USGS.flow.data$datetime)
#you can use this data$datetime (copy paste into console) to pull out the data for this specific column
...
> Answer: Blank cells stay blank if they're factor/characters, but are turned into NAs if they're numeric.
...
> Answer: correlation matrices get messed up by NAs. If you remove NAs, entire rows will be gone. 

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data)
#rename dataframe so you're not editing the original. 
dim(USGS.flow.data)
dim(USGS.flow.data.complete)
#rows removed from new dataframe
...
# summary doesn't give SD, have to code it separately.
...
In some cases when dates are provided as integers, you may need to provide an origin for your dates. Beware: the "origin" date for Excel (Windows), Excel (Mac), R, and MATLAB all have different origin dates. Google this if it comes up. R is 1 Jan 1970
...
#what exists in the data now (as.Date)
```
!!!
