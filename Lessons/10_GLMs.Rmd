...
EPAair.subsample <- sample_n(EPAair, 5000) # BC A SHAPIRO TEST CAN ONLY USE 5000 SAMPLES

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) #p-val is sig = not normal distribution
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram()  #right skewed - has right tail
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) #what the data would look like if it were normal

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample 
#mu is the mean of 50, alternative hyp is that the mean is less than 50 - set the alt so that the t-test doesn't assume both greater than or less than - turn it into a one-tailed t-test instead of a two-sided t-test. Null hyp is rejected; p-val is sig; if the t-test is less than -1.96 or greater than 1.96, then the p-val will be sig. the true mean is between -infinity and 41.13416, because it doesn't overlap with 50, that means it's sig different. And, the mean is also within that CI. 
...
Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  ...
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #sets axes at 0,0
print(Ozone.plot)
#note that you can go between count and density with stat = "count"; density is kind of a moving avg - proportion of total sample at that given sample. 
```
...
> A one-tailed t-test was done with the alt hyp that the mean is less than 50; the results show that yes, the mean is significantly less than 50, with a low p-value, a t-test value less than -1.96, and a mean within the confidence interval which were also less than 50. 
"EPA ozone measurements for 2018-2019 were significantly lower than 50, the AQI threashold for "good" air quality (t-test; t = -57.98. p< 0.0001)."  - leave out alt hyp or null hyp, don't list the t-stat results. 
...
```{r}
#was aq sig different in 2018 vs 2019?
shapiro.test(EPAair$Ozone[EPAair$Year == 2018]) 
shapiro.test(EPAair$Ozone[EPAair$Year == 2019])
#neither year's distribution is normal
var.test(EPAair$Ozone ~ EPAair$Year) # ~ = by 
#the variances between the years are not equal; significantly different??? don't understand... 
...
# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year) #~ instead of mu, just do the alt that the means are different. 
O3.twosample
O3.twosample$p.value 
#the difference is not equal to zero, with the difference in means being somewhere between -1.46 and -0.22. Degrees of freedom = over 6k, means there can be more variables added. 
...
plot(O3.twosample2) #hit enter in the console; residuals vs fitted 
#par(mfrow=c(2,2)) #will plot all the charts at once, but will apply to all future plots too. 
...
```{r}
Ozone.plot <- ggplot(EPAair, aes(x = Ozone,  color = as.factor(Year))) +
  geom_density()+
  scale_color_manual(name = "Year", values = c("2018" = "slateblue3", "2019" = "darkgoldenrod2", alpha = 0.3))+
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #sets axes at 0,0
print(Ozone.plot)


Ozone.boxplot <- function(ystuff) {
  ystuff <- enquo(ystuff)
  ggplot(EPAair, aes(x = Ozone, !!ystuff, color = as.factor(Year))) +
  geom_boxplot()+
