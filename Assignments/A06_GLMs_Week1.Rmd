...
#NOTE: the HSD tests are superfluous because the interaction didn't exist. Really, when groupings are done, as can be seen in the figures below, the only paramter that matters is which lake has the higher values for TN/TP. This is Peter in every case. The figures reflect this by having just an a/b labeling scheme. 
...
```{r}
#7
tn.anova.plot <- ggplot(ppp_nut_cleaned, aes(y = tn_ug, x = month, color =  lakename)) +
  geom_boxplot()+
stat_summary(geom = "text", fun.y = max, vjust = -1, size = 3.5,
             label = c("a", "b", "a", "b","a","b","a","b","a", "b"), position = position_dodge(0.75), show.legend=FALSE)+
...
tp.anova.plot <- ggplot(ppp_nut_cleaned, aes(y = tp_ug, x = month, color =  lakename)) +
  geom_boxplot()+
stat_summary(geom = "text", fun.y = max, vjust = -1, size = 3.5,
             label = c("a", "b", "a", "b","a","b","a","b","a", "b"), position = position_dodge(0.75))+
  scale_color_viridis_d(option = "inferno", begin = 0.2, end = 0.8)+
