...
> Answer: Wow. The model explains 78% of the variance. That's crazy high! Final set of explanatory variables that predict temp: lakes (crampton lake, east long lake, hummingbird lake, paul lake, peter lake, tuesday lake, ward lake, and long lake), year, day within July, and depth. The only lake that didn't have a specific coefficient listed was Central Long Lake. 

6. Run an interaction effects ANCOVA to predict temperature based on depth and lakename from the same wrangled dataset.

```{r, message = FALSE}
#6
cpancova.interaction <- lm(data = cp, temperature_C ~ lakename * depth)
summary(cpancova.interaction)
```
...
```{r, message = FALSE}
#8
cpplot <- ggplot(cp, aes(x = depth, y = temperature_C, color = lakename)) + 
  geom_point() + 
