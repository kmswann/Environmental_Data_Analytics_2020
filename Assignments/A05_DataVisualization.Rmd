...
```{r}
#1
ppl_n <- read.csv("./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaul_Processed.csv")
ppl_cp <- read.csv("./Data/Processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv")
litter <- read.csv("./Data/Processed/NEON_NIWO_Litter_mass_trap_Processed.csv")

#2
class(ppl_cp$sampledate)
class(ppl_n$sampledate)
ppl_cp$sampledate <- as.Date(ppl_cp$sampledate, format = "%m/%d/%Y")
ppl_n$sampledate <- as.Date(ppl_n$sampledate, format = "%m/%d/%Y")
class(ppl_cp$sampledate)
class(ppl_n$sampledate)
