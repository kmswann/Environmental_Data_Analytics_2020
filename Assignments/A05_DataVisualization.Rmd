...
blahtheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "salmon"), axis.ticks = element_line(colour = 'darkseagreen4', size = 2, linetype = 'dashed'), axis.ticks.length = unit(1, "cm"),
        legend.position = "top", plot.background= element_rect(color="gold")) #I'm sorry, but 'laser' and 'canary' should totally be colors. 
...
pxp <- ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = po4, color=lakename))+
 ...
 labs( x = expression(TP ~ (mu*g / L)), y = expression(PO[4]^{"3-"})) 
...
plot_wq <- function(xaes) {
  xaes <- enquo(xaes)
  ggplot(PeterPaul.chem.nutrients, aes(!!xaes, month)) +
    geom_boxplot() +
...
prow <- plot_grid(
  ...
  align = 'vh',
   labels = c("Temp (C)", expression(TP ~ (mu*g / L)),expression(TN ~ (mu*g / L)),
  hjust = -1,
  nrow = 1
))
print(prow)
